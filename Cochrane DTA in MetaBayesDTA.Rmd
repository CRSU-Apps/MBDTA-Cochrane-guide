---
title: <center> Reproducing analyses from the Cochrane DTA handbook in MetaBayesDTA <br> v1.0
author: <center> Tom Morris (tm428@leicester.ac.uk)
date: <center> October 2023
output: 
...
This guide contains step-by-step instructions on how to reproduce the example meta-analyses in the Cochrane Handbook for Systematic Reviews of Diagnostic Test Accuracy Version 2 <span style="text-decoration:underline"> [(https://training.cochrane.org/handbook-diagnostic-test-accuracy/current)](https://training.cochrane.org/handbook-diagnostic-test-accuracy/current)</span>, using the MetaBayesDTA app <span style="text-decoration:underline">  [(https://crsu.shinyapps.io/MetaBayesDTA/)](https://crsu.shinyapps.io/MetaBayesDTA/)</span>. Six of the examples contained in chapters 9 and 10 of the handbook can be carried out in MetaBayesDTA. Before starting, the reader should familiarise themselves with the example(s) they wish to reproduce and download the accompanying datasets.
...
The two drop-down menus contain Stan sampling options and prior distributions. They can both be left at their default values. Run sampling from the prior distributions by clicking **Click to run prior model**.

<br>

```{r, label = "", dpi=45, fig.align = "center", echo = FALSE, fig.cap ="", out.height="90%", out.width="90%"}
knitr::include_graphics(".//Cochrane DTA 9.4.2 example 1//Pictures//Analysis 2.png")
```

<br>

A text box will appear. Scroll down and click **OK**.

<br>

```{r, label = "", dpi=45, fig.align = "center", echo = FALSE, fig.cap ="", out.height="90%", out.width="90%"}
knitr::include_graphics(".//Cochrane DTA 9.4.2 example 1//Pictures//Analysis 3.png")
```

<br>

When the sampling has finished, details of the prior distributions will appear in a table and several plots.

<br>

```{r, label = "", dpi=45, fig.align = "center", echo = FALSE, fig.cap ="", out.height="90%", out.width="90%"}
knitr::include_graphics(".//Cochrane DTA 9.4.2 example 1//Pictures//Analysis 4.png")
```

<br>

Navigate to the **Run model** tab.
...
Another text box will pop up. Click **OK** to close it.
...
<!--------------------------------------------------------------------------------------->
## Estimate the parameters (optional)
...
MetaBayesDTA does not currently produce estimates of all of the parameters in the Cochrane handbook, and provides only median parameter estimates and posterior intervals for those that it does. However, all the output in the handbook can be estimated from MetaBayesDTA output. This is done for each parameter in the following subsections, with justification for the calculations provided in the appendix.
...
#sigmaA
sigmaA_1 <- 0.765
#Standard deviation of muA
sdMuA_1 <- sigmaA_1 / sqrt(n_1)
#Data frame to put the estimates in a table
dfMuA_1 <- data.frame(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), est=c(0.6534, 0.6536, muA_1), stdDev=c(0.1275, 0.1288, format(sdMuA_1, digits=4)), lower=c(NA, 0.4037, 0.398), upper=c(NA, 0.9100, 0.903))
...
Referring to the Parameter Estimates tab in MetaBayesDTA, logit sensitivity can be read straight off, `r muA_1`, as can its 95% posterior interval. The standard deviation can be estimated by:

$$SD(\mu_A) = `r sigmaA_1` / \sqrt{`r n_1`} = `r format(sdMuA_1, digits=4)`$$

where `r sigmaA_1` is $\sigma_A$ ($\sigma_1$ in MetaBayesDTA) and `r n_1` is the number of trials.
...
#sigmaB
sigmaB_1 <- 0.786
#Standard deviation of muB
sdMuB_1 <- sigmaB_1 / sqrt(n_1)
#Data frame to put the estimates in a table
dfMuB_1 <- data.frame(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), est=c(3.1090, 3.1028, muB_1), stdDev=c(0.1459, 0.1454, format(sdMuB_1, digits=4)), lower=c(NA, 2.8282, 2.815), upper=c(NA, 3.4031, 3.405))
#Table
knitr::kable(x=dfMuB_1, col.names=c("", "Estimate", "Standard deviation", "Lower 95% limit", "Upper 95% limit"), align = 'l')
```


Logit specificity can be read straight off, `r muB_1`, as can its 95% posterior interval. The standard deviation can be estimated by:

$$SD(\mu_B) = `r sigmaB_1` / \sqrt{`r n_1`} = `r format(sdMuB_1, digits=4)`$$

where `r sigmaB_1` is $\sigma_B$ ($\sigma_0$ in MetaBayesDTA) and `r n_1` is the number of trials.
...
#Lower limit
varAL_1 <- sigmaAL_1^2
#Estimate of standard deviation of (Variance of logit sensitivity) using upper limit
sdVarAU_1 <- sigmaAU_1 * sqrt(2 * (n_1 - 1)) * qchisq(p=0.025, df=n_1-1) / (n_1 - 1)^2
#...using lower limit
sdVarAL_1 <- sigmaAL_1 * sqrt(2 * (n_1 - 1)) * qchisq(p=0.975, df=n_1-1) / (n_1 - 1)^2
#...using both limits
sdVarA_1 <- (sdVarAU_1 + sdVarAL_1) / 2
#Data frame to put the estimates in a table
dfVarA_1 <- data.frame(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), est=c(0.5426, 0.5318, format(varA_1, digits=4)), stdDev=c(0.1463, 0.1511, format(sdVarA_1, digits=4)), lower=c(NA, 0.3276, format(varAL_1, digits=4)), upper=c(NA, 0.9125, format(varAU_1, nsmall=4)))
#Table
...
$$(`r sigmaAL_1` ^2, `r format(sigmaAU_1, nsmall=3)` ^2) = (`r format(sigmaAL_1^2, digits=3)`, `r format(sigmaAU_1^2, nsmall=3)`).$$
  
The standard deviation of $\sigma_A^2$ can be estimated from the 95% posterior interval in MetaBayesDTA using the upper limit, `r format(sigmaAU_1, nsmall=3)`: 

$$SD(\sigma_A^2) = `r format(sigmaAU_1, nsmall=3)` \times \sqrt{2 \times `r n_1 - 1`} \times `r format(qchisq(p=0.025, df=n_1-1), digits=4)` / `r n_1 - 1`^2 = `r format(sdVarAU_1, digits=3, nsmall=3)`.$$
  
    
Here `r n_1-1` is the number of trials minus 1, and `r format(qchisq(p=0.025, df=n_1-1), digits=4)` is the 2.5% quantile of the $\chi_{`r n_1-1`}^2$ distribution.

Similarly, using the lower limit, `r sigmaAL_1`:
  
$$SD(\sigma_A^2) = `r sigmaAL_1` \times \sqrt{2 \times `r n_1 - 1`} \times `r format(qchisq(p=0.975, df=n_1-1), digits=4)` / `r n_1-1`^2 = `r format(sdVarAL_1, digits=3)`$$
  
where `r format(qchisq(p=0.975, df=n_1-1), digits=4)` is the 97.5% quantile of the $\chi_{`r n_1-1`}^2$ distribution. There is a large discrepancy in these two estimates. This implies that the posterior distribution does not have the Chi-squared distribution that would be assumed in a frequentist model. It appears that the estimate using the lower limit is better, but in practice one may not have another estimate to compare against. Therefore, without access to the posterior distribution itself, the mean of the two estimates can be used as the overall estimate:
  
$$SD(\sigma_A^2) = (`r format(sdVarAU_1, digits=3, nsmall=3)` + `r format(sdVarAL_1, digits=3)`) / 2 = `r format(sdVarA_1, digits=3)`.$$
...
#Lower limit
varBL_1 <- sigmaBL_1^2
#Estimate of standard deviation of (Variance of logit specifitiy) using upper limit
sdVarBU_1 <- sigmaBU_1 * sqrt(2 * (n_1 - 1)) * qchisq(p=0.025, df=n_1-1) / (n_1 - 1)^2
#...using lower limit
sdVarBL_1 <- sigmaBL_1 * sqrt(2 * (n_1 - 1)) * qchisq(p=0.975, df=n_1-1) / (n_1 - 1)^2
#...using both limits
sdVarB_1 <- (sdVarBU_1 + sdVarBL_1) / 2
#Data frame to put the estimates in a table
...
The standard deviation of $\sigma_B^2$ can be estimated from the 95% posterior interval using the upper limit, `r format(sigmaBU_1, nsmall=3)`: 

$$SD(\sigma_B^2) = `r format(sigmaBU_1, nsmall=3)` \times \sqrt{2 \times `r n_1 - 1`} \times `r format(qchisq(p=0.025, df=n_1-1), digits=4)` / `r n_1 - 1`^2 = `r format(sdVarBU_1, digits=3, nsmall=3)`$$

and the lower limit, `r sigmaBL_1`:
  
$$SD(\sigma_B^2) = `r sigmaBL_1` \times \sqrt{2 \times `r n_1 - 1`} \times `r format(qchisq(p=0.975, df=n_1-1), digits=4)` / `r n_1-1`^2 = `r format(sdVarBL_1, digits=3)`.$$
  
Again, there is a large discrepancy. This time the mean appears to be a better estimate than the one using the lower limit. The mean of the estimates is
  
$$SD(\sigma_B^2) = (`r format(sdVarBU_1, digits=3, nsmall=3)` + `r format(sdVarBL_1, digits=3)`) / 2 = `r format(sdVarB_1, digits=3)`.$$
...
covFreq_1 <- -0.2704
#Correlation of logit sensitivity and logit specificity
rho_1 <- -0.414
#Upper limit of 95% posterior interval
rhoU_1 <- -0.027
#Lower limit
rhoL_1 <- -0.688
#Standard deviation of correlation
sdRho_1 <- (1 - rho_1^2) / sqrt(n_1-1)
...
#Data frame to put the estimates in a table
dfRho_1 <- data.frame(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), est=c(format(rhoF_1, digits=4), -0.4362, rho_1),  stdDev=c(format(seRhoF_1, digits=4), 0.1567, format(sdRho_1, digits=3)), lower=c(NA, -0.6993, rhoL_1), upper=c(NA, -0.0911, rhoU_1))
#Table
...
where `r n_1-1` is the number of trials minus 1. The same calculation can be used to estimate the standard deviation of $\rho$ from the MetaBayesDTA output:

$$SD(\rho) = (1 - (`r format(rho_1, digits=4)`)^2) / \sqrt{`r n_1-1`} = `r format(sdRho_1, digits=3)`.$$
...
#Lower limit
betaL_2 <- -0.097
#Estimate of SD(beta) from sensitivity equation and upper limit of posterior interval
sdBetaSensU_2 <- exp((betaU_2 - beta_2) / 2) - 1
#Estimate of SD(beta) from sensitivity equation and lower limit of posterior interval
sdBetaSensL_2 <- 1 - exp((betaL_2 - beta_2) / 2)
#Estimate of SD(beta) from specificity equation and upper limit of posterior interval
sdBetaSpecU_2 <- 1 - exp((beta_2 - betaU_2) / 2)
#Estimate of SD(beta) from specificity equation and lower limit of posterior interval
sdBetaSpecL_2 <- exp((beta_2 - betaL_2) / 2) - 1
#Combined estimate of SD(beta)
sdBeta_2 <- mean(c(sdBetaSensU_2, sdBetaSensL_2, sdBetaSpecU_2, sdBetaSpecL_2))
#Data frame to put the estimates in a table
dfBeta_2 <- data.frame(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), est=c(0.2267, 0.2189, beta_2), stdDev=c(0.1624, format(0.1630, nsmall=4), format(sdBeta_2, digits=4)),  lower=c(NA, -0.1066, betaL_2), upper=c(NA, 0.5348, betaU_2))
#Table
knitr::kable(x=dfBeta_2, col.names=c("", "Estimate", "Standard deviation", "Lower 95% limit", "Upper 95% limit"), align = 'l')
```

There are four ways to estimate the standard deviation of $\beta$, since there is a sensitivity equation (defined by $j = 1$ in the model definition) and a specificity equation ($j = 2$), and two limits of the posterior interval. These four estimates are

\begin{align*}
\text{Sensitivity equation, upper limit: } \hspace{5pt} & \exp{((`r betaU_2` - `r beta_2`) / 2)} - 1 & = `r format(sdBetaSensU_2, digits=4)` \\
\text{Sensitivity equation, lower limit: } \hspace{5pt} & 1 - \exp{((`r betaL_2` - `r beta_2`) / 2)} & = `r format(sdBetaSensL_2, digits=4)` \\
\text{Specificity equation, upper limit: } \hspace{5pt} & 1 - \exp{((`r beta_2` - `r betaU_2`) / 2)} & = `r format(sdBetaSpecU_2, digits=4)` \\
\text{Specificity equation, lower limit: } \hspace{5pt} & \exp{((`r beta_2` - `r betaL_2`) / 2)} - 1 & = `r format(sdBetaSpecL_2, digits=4)` \\
\end{align*}

The mean of the estimates can be taken as the overall estimate:

$$ (`r format(sdBetaSensU_2, digits=4)` + `r format(sdBetaSensL_2, digits=4)` + `r format(sdBetaSpecU_2, digits=4)` + `r format(sdBetaSpecL_2, digits=4)`) / 4 = `r format(sdBeta_2, digits=4)`$$
...
#Data frame to put the estimates in a table
dfMuANuA <- data.frame(parameter=c("$\\mu_A$", " ", " ", "$\\nu_A$", " ", " "), rep(c("Cochrane frequentist", "Cochrane Bayesian", "MetaBayesDTA"), times=2), est=c(-0.0965, -0.1001, muA_3, 0.9626, 0.9685, nuA_3), stdDev=c(0.2203, 0.2287, format(sdMuA_3, digits=4), 0.2513, 0.2605, format(sdNuA_3, digits=4)), lower=c(NA, -0.5547, muAL_3, NA, 0.4634, format(nuAL_3, digits=4, nsmall=4)), upper=c(NA, 0.3478, muAU_3, NA, 1.4906, format(nuAU_3, digits=4, nsmall=4)))
...
MetaBayesDTA provides the mean logit sensitivity for the two subgroups, which are $(\mu_1L=CCP1) = \mu_A$ in the CCP1 group and $(\mu_1L=CCP2) = \mu_A + \nu_A$ in the CCP2 group. The parameter estimate and posterior interval for $\mu_A$ can be read from the output for $(\mu_1L=CCP1)$. The standard deviation of $\mu_A$ is

$$SD(\mu_A) = `r format(sigmaA_3, nsmall=3)` / \sqrt{`r n1_3`} = `r format(sdMuA_3, digits=4)`$$

where $`r format(sigmaA_3, nsmall=3)`$ is $\sigma_A$ ($\sigma_1$ in MetaBayesDTA) and $`r n1_3`$ is the number of trials in the CCP1 group.
...
where `r n2_3` is the number of trials in the CCP2 group and `r n1_3 + n2_3` is the total number of trials.
...
knitr::include_graphics(".//Cochrane DTA 9.4.6.4 example 2//Pictures//SROC.png")
```
...
```{r, label = "", dpi=45, fig.align = "center", echo = FALSE, fig.cap ="", out.height="90%", out.width="90%"}
knitr::include_graphics(".//Cochrane DTA 9.4.6.4 example 2//Pictures//SROC 2.png")
```
...
This sections refers to $\mu_A$ and $\mu_B$ in the bivariate model, and $\Lambda$ and $\Theta$ in the HSROC model.

Assuming the parameter $\mu$ is the mean of a Normal distribution $N(\mu, \sigma^2)$ and has been estimated from $n$ trials, the standard deviation of $\mu$ can be estimated by
...
where $E(\sigma^2) = \sigma_m^2$ and $n$ is the number of trials, its standard deviation can be estimated as

$$SD(\sigma^2) = U \sqrt{2(n-1)} \chi^2_{n-1:0.025} / (n-1)^2$$

where $U$ is the upper limit of the posterior interval and $n$ is the number of trials. Using the lower limit of the posterior interval $L$, the corresponding estimate is

$$SD(\sigma^2) = L \sqrt{2(n-1)} \chi^2_{n-1:0.975} / (n-1)^2$$

These formulare are derived as follows. With the distributional assumption above, a 95% confidence interval for $\sigma^2 (n-1) / \sigma_m^2$ is

$$(\chi^2_{n-1;0.025}, \chi^2_{n-1;0.975}).$$

Dividing by $\sigma^2 (n-1)$ yields a 95% confidence interval for $1 / \sigma_m^2$ of

$$(\chi^2_{n-1;0.025} / (\sigma^2 (n-1)), \chi^2_{n-1;0.975} / (\sigma^2 (n-1))),$$

and taking reciprocals gives a 95% confidence interval for $\sigma_m^2$ of

$$(\sigma^2 (n-1) / \chi^2_{n-1;0.975}, \sigma^2 (n-1) / \chi^2_{n-1;0.025}),$$

taking care to switch the lower and upper limits.


Next, given $\sigma^2 \sim \sigma_m^2 \chi^2_{n-1} / (n-1)$ we have

$$\begin{align*}
SD(\sigma^2) & = SD(\sigma_m^2 \chi^2_{n-1} / (n-1)) \\
& = \sigma_m^2 SD(\chi^2_{n-1}) / (n-1) \\
& = \sigma_m^2 \sqrt{2(n - 1)} / (n-1)
\end{align*}$$

since $Var(\chi^2_{n-1}) = 2(n-1)$. Therefore to obtain an estimate of $SD(\sigma^2)$ from the upper limit 

$$U = \sigma^2 (n-1) / \chi^2_{n-1;0.025}$$

of the posterior interval, $U$ must be multiplied by a constant $C$ satisfying

$$UC = \sigma^2 \sqrt{2(n - 1)} / (n-1).$$

$C$ can be calculated as

$$\begin{align*}
C & = \sigma^2 \sqrt{2(n - 1)} / U(n-1) \\
& = \sigma^2 \sqrt{2(n - 1)} / [\sigma^2 (n-1) / \chi^2_{n-1;0.025}](n-1) \\
& = \sigma^2 \sqrt{2(n - 1)} \chi^2_{n-1;0.025} / \sigma^2 (n-1)^2 \\
& = \sqrt{2(n - 1)} \chi^2_{n-1;0.025} / (n-1)^2.
\end{align*}$$

Therefore the estimate of $SD(\sigma^2)$ is $UC = U \sqrt{2(n - 1)} \chi^2_{n-1;0.025} / (n-1)^2$. 

A similar calculation holds for the lower limit $L$.
...
This sections refers to $\beta$ in the HSROC model. Its standard deviation can be estimated in four ways, using the sensitivity equation

$$logit(\pi_{i1}) = (\theta_i + \alpha_i / 2) \exp{(-\beta / 2)}$$

or the (1 minus) specificity equation

$$logit(\pi_{i2}) = (\theta_i - \alpha_i / 2) \exp{(\beta / 2)},$$

and using the lower limit $L$ or upper limit $U$ of the posterior interval. The four estimates are

\begin{align*}
\text{Sensitivity equation, upper limit: } \hspace{5pt} & \exp{((U - \hat{\beta}_m) / 2)} - 1 \\
\text{Sensitivity equation, lower limit: } \hspace{5pt} & 1 - \exp{((L - \hat{\beta}_m) / 2)} \\
\text{Specificity equation, upper limit: } \hspace{5pt} & 1 - \exp{((\hat{\beta}_m - U) / 2)} \\
\text{Specificity equation, lower limit: } \hspace{5pt} & \exp{((\hat{\beta}_m - L) / 2)} - 1
\end{align*}

where $\hat{\beta}_m$ is the posterior median of $\beta$. The mean of these estimates can be used as the overall estimate.

Since the derivation of the estimates are similar, only one is shown, the sensitivity equation using the upper limit.

The sensitivity equation can be rearranged into 

$$\exp{(\beta / 2)} = (\theta_i + \alpha_i / 2) / logit(\pi_{i1}).$$

The parameters $\theta_i$ and $\alpha_i$ are assumed to be independent and Normally distributed, and the mean sensitivities $\pi_{i1}$ are regarded as constants. Therefore $\exp{(\beta / 2)}$ is a linear combination of independent Normal distributions, and so is also Normal, say $\exp{(\beta / 2)} \sim N(\mu, \sigma^2)$. An estimate of $\mu$ can be obtained by utilising the fact that $\mu$ is the median of the distribution of $\exp{(\beta / 2)}$; it is not the case that $E(\exp{(\beta / 2)}) = \exp{(E(\beta / 2))}$, but this equality does hold if the expected values are replaced with medians. Therefore:

$$\hat{\mu} = \exp{(\hat{\beta}_m / 2)}.$$

A confidence interval for $\mu$ is $\hat{\mu} \pm 1.96 \hat{\sigma}$, which can be transformed into a confidence interval for $\beta_m$ with limits $L = 2 log( \hat{\mu} - 1.96 \hat{\sigma} )$ and $U = 2 log( \hat{\mu} + 1.96 \hat{\sigma} )$, as long as $\hat{\mu} - 1.96 \hat{\sigma} > 0$. An estimate of $\sigma$ in terms of $U$ and $\hat{\mu}$ can then be found by rearranging the equation for $U$:

$$\hat{\sigma} = (\exp{(U / 2)} - \hat{\mu}) / 1.96.$$

In general terms, given a random variable $X$ and a function $g$, the variance of $g(X)$ can be approximated as $$Var(g(X)) \approx (g'(E(X)))^2 Var(X).$$

Putting $X = \exp(\beta / 2)$ and $g(x) = 2 log(x)$ and observing that $g'(x) = 2 / x$ yields

$$Var(\beta) = Var(2 log( \exp{(\beta / 2)} )) \approx (2 / \hat{\mu})^2 \hat{\sigma}^2 = (2 \hat{\sigma} / \hat{\mu})^2.$$

Therefore

$$SD(\beta) \approx 2 \hat{\sigma} / \hat{\mu}.$$

Substituting in the estimates $\hat{\mu}$ and $\hat{\sigma}$ obtained earlier gives

\begin{align*}
SD(\beta) & \approx 2 \hat{\sigma} / \hat{\mu} \\
& = 2((\exp{(U / 2)} - \hat{\mu}) / 1.96) / \hat{\mu} \\
& \approx (\exp{(U / 2)} - \hat{\mu}) / \hat{\mu} \\
& = (\exp{(U / 2)} / \hat{\mu}) - 1 \\
& = (\exp{(U / 2)} / \exp{(\hat{\beta}_m / 2)}) - 1 \\
& = (\exp{((U / 2) - (\hat{\beta}_m / 2))} - 1 \\
& = \exp{((U - \hat{\beta}_m) / 2)} - 1.
\end{align*}
...
This sections refers to $\mu_A$, $\mu_B$, $\nu_A$ and $\nu_B$ in the bivariate model with a covariate.

In 9.4.6.3 example 1 the handbook gives parameter estimates for $\mu$ and $\nu$ (omitting the subscript $A$ or $B$), whereas MetaBayesDTA gives estimates for the mean logit sensitivity (and specificity) within the two subgroups, namely $\mu_1 = \mu$ for group 1 (CCP1) and $\mu_2 = \mu + \nu$ for group 2 (CCP2). This subsection describes how to obtain an estimate of the standard deviaton of $\mu$, and a point estimate, standard deviation and 95% posterior interval for $\nu$.

Let $n_i$ be the number of trials in group $i$ with total number of trials $n = n_1 + n_2$. The standard deviation of $\mu$ is estimated by

$$SD(\mu) = \hat{\sigma} / \sqrt{n_1}$$

A point estimate of $\nu$ is $\hat{\nu} = \hat{\mu}_2 - \hat{\mu}_1$, and its standard deviation can be estimated by
...
The derivation of these estimates is now given. Focusing on the marginal distribution of sensitivity or specificity, the model is described by $Y_i \sim N(\mu + \nu Z_i, \sigma^2)$, where $Z_i = 0$ in group 1 and $Z_i = 1$ in group 2. Writing the model in its usual matrix form
